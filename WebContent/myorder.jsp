...
<script type="text/javascript">
	$(function() {
		...
		if ($status.length == 0) {
			//全部
			$(".ostatus>ul>li>a[href='myorder.action']").parent().css(
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-sm-9 col-md-9">
				...
				<div class="ostatus">
					<ul>
						<li><a href="myorder.action">全部</a></li>
						<li><a href="status_option.action?status=-1">已撤销</a></li>
						<li><a href="status_option.action?status=0">交易中</a></li>
						<li><a href="status_option.action?status=2">交易完成</a></li>
					</ul>
				</div>
				<!-- 订单状态菜单 -->
				...
				<div style="width: 96%; margin-left: 19px;" class="dlist">
					<form name="addrForm" id="addrForm" action="EnsureOrderServlet"
						...
						<table cellspacing="0" cellpadding="0" class="order-table"
							...
							<tbody class="J_Shop">
								<!-- forEach开始 -->
								<c:forEach items="${requestScope.oilist}" var="orderItem">
									<tr class="item">
										<td class="s-title"><a href="#" class="J_MakePoint">
												<img src="${orderItem.product.fileName }" class="itempic">
												<span class="title J_MakePoint">${orderItem.product.account.game_name }：${orderItem.product.product_name }</span>
										</a>
											<div class="props">
												<span>游戏区服：</span> <span>${orderItem.product.account.game_region }
												</span> <span>${orderItem.product.account.game_server } </span>
											</div>
											<div>
												<!-- 显示商品描述 -->
												<span style="color: gray;">简介：${orderItem.product.describe }</span>
											</div></td>
										<td class="s-price">
											<!-- 总价 --> <span class='price'> <em
												class="style-normal-small-black J_ItemPrice" id="price">${orderItem.product.price * orderItem.count}元</em>
										</span> <span>${orderItem.order.shopDate }</span>
										</td>
										<td class="s-amount">
											<!-- 购买数量 --> ${orderItem.count }
										</td>
										<td class="s-total" style="padding-right: 10px">
											<!-- 订单状态 --> <c:choose>
												<c:when test="${orderItem.order.status==0 }">
													<!-- 把选中商品的id，购买数量，订单id，传到对应的servlet -->
													<a
														href="order_option.action?proid=${orderItem.product.id}&count=${orderItem.count }&orderid=${orderItem.order.id}&option=cel"
														class="add-btn"
														style="height: 22px; width: 56px; padding: 0px 3px 0px 0px">撤销订单</a>
													<a
														href="order_option.action?proid=${orderItem.product.id}&count=${orderItem.count }&orderid=${orderItem.order.id}&option=en"
														class="add-btn"
														style="height: 22px; width: 56px; padding: 0px 3px 0px 0px">确认收货</a>
													<span>&nbsp;交易中</span>
												</c:when>
												<c:when test="${orderItem.order.status==2 }">
													<span>交易完成</span>
												</c:when>
												<c:when test="${orderItem.order.status==-1 }">
													<span>已撤销</span>
												</c:when>
											</c:choose>
